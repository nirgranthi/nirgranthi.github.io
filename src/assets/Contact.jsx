...
export const Contact = () => {
  ...
  return (
    <section id="contact" className={`py-24 px-6 relative overflow-hidden ${isNeo ? 'bg-[#FFFDF6] border-t-4 border-black' : isSwiss ? 'bg-white border-t border-black' : 'bg-slate-950'}`}>
      <BackgroundGrid />
      ...
      <div className="max-w-5xl mx-auto relative z-10 text-center">
        ...
        <motion.p
          className={isNeo || isSwiss ? "text-black/85 font-bold mb-16 text-lg max-w-xl mx-auto" : "text-slate-400 mb-16 text-lg max-w-xl mx-auto"}
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
              ...
              <div className="flex flex-col items-center gap-5">
                ...
                <div>
                  ...
                  {link.href ? (
                    ...
                    <div className={isNeo || isSwiss ? "text-black font-black text-lg flex items-center justify-center gap-2" : "text-white font-bold text-lg flex items-center justify-center gap-2"}>
                      {link.value}
                      {copied ? (
                        <Check size={18} className="text-green-600" />
                      ) : (
                        <Copy size={18} className={isNeo || isSwiss ? "text-black/60 opacity-0 group-hover:opacity-100 transition-opacity" : "text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity"} />
                      )}
                    </div>
                  )}
                </div>
